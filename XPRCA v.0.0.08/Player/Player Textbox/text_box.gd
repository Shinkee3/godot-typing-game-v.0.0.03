...
func _physics_process(delta: float) -> void: # change this to signal bus
	if PlayerInfo.indialogue == true && dialogueIndicator.visible == false:
		dialogueIndicator.show()
	elif PlayerInfo.indialogue == false && dialogueIndicator.visible == true:
		dialogueIndicator.hide()

func _unhandled_key_input(event: InputEvent) -> void:
	if PlayerInfo.indialogue != true:
		if dialogueIndicator.visible == true:
			dialogueIndicator.hide()
